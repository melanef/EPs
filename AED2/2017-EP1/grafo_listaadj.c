...
{
    int i, verticeOrigem, verticeDestino, peso;
...
{
    TipoApontador arestaAtual;

    if (listaAdjVazia(v1, grafo)) {
        return false;
    }

    arestaAtual = primeiroListaAdj(v1, grafo);
    while (arestaAtual->prox != NULL) {
...
{
    TipoApontador arestaAtual, arestaAnterior;
...
void imprimeGrafo(TipoGrafo *grafo)
{
    ...
    for (i = 1; i <= grafo->numVertices; i++) {
        aresta = primeiroListaAdj(i, grafo);
        while (aresta != NULL) {
...
void imprimeConjuntosConectados(TipoLista *conjuntos)
{
    ...
    int i = 0;
    if (!listaVazia(conjuntos)) {
        TipoNo *atual = conjuntos->inicio;
        while (atual != NULL) {
            TipoLista *endereco = (TipoLista *) (uintptr_t) atual->v;
            atual = atual->prox;
            printf("C%d: ", ++i);
            imprimeLista(endereco);
            printf("\n");
        }
    }
...
{
    int i;
    int quantidadeConjuntos = contaLista(conjuntos);
    TipoLista *vertices = criaLista();

    for (i = 0; i < grafo->numVertices; i++) {
        int atual = i + 1;
        TipoLista *conjuntosSemVerticeAtual = conjuntosConectadosSemVertice(grafo, atual);
        if (contaLista(conjuntosSemVerticeAtual) > quantidadeConjuntos) {
            insereOrdenado(atual, vertices);
        }
    }
...
TipoLista *conjuntosConectadosSemVertice(TipoGrafo *grafo, int excluido)
{
    ...
    for (i = 0; i < grafo->numVertices; i++) {
        int atual = i + 1;
        if (atual != excluido && cores[atual - 1] == COR_BRANCO) {
            TipoLista *fila = criaLista();
            visitaConjuntosConectadosSemVertice(atual, grafo, cores, fila, excluido);
            insereFila((uintptr_t) fila, conjuntos);
        }
    }
...
{
    if (v != excluido && cores[v - 1] == COR_BRANCO) {
        cores[v - 1] = COR_CINZA;
        insereOrdenado(v, fila);
        ...
        if (!listaAdjVazia(v, grafo)) {
            TipoApontador aresta = primeiroListaAdj(v, grafo);
            while (aresta != NULL) {
                if (aresta->vdest != excluido) {
                    visitaConjuntosConectadosSemVertice(aresta->vdest, grafo, cores, fila, excluido);
                }
